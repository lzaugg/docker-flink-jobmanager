...
if [ ! "${FLINK_CONF}x" = "x" ]; then
	...
	ruby $bin/docker_merge-yml-file.rb /tmp/flink-conf-orig.yaml /tmp/flink-conf-env.yaml | grep -v '\---'|sed -e 's/"//g' > /tmp/flink-conf.yaml ; ( exit ${PIPESTATUS[0]} )
	if [ ! "$?" = "0" ]; then
...
if [ ! "${HADOOP_CORE_CONF}x" = "x" ]; then
	...
	ruby $bin/docker_merge-xml-with-yaml.rb ${CORE_SITE_CONFIG_PATH}-orig /tmp/${CORE_SITE_CONFIG_FILENAME}-env > /tmp/${CORE_SITE_CONFIG_FILENAME} ; ( exit ${PIPESTATUS[0]} )
	if [ ! "$?" = "0" ]; then
...
if [ ! "${HADOOP_HDFS_CONF}x" = "x" ]; then
	...
	ruby $bin/docker_merge-xml-with-yaml.rb ${HDFS_SITE_CONFIG_PATH}-orig /tmp/${HDFS_SITE_CONFIG_FILENAME}-env > /tmp/${HDFS_SITE_CONFIG_FILENAME} ; ( exit ${PIPESTATUS[0]} )
	if [ ! "$?" = "0" ]; then
